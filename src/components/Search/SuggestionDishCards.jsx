...
import { SEARCH_DISH_IMG_URL } from "../../utils/API";
...
const DishCards = ({ item }) => {
  ...
  return (
    <div className="bg-white px-4 pt-5 pb-10 rounded-3xl">
      ...
      <Link to={`/restaurant/${id}`} className="flex justify-between items-center pb-3 border-dotted border-b-[2px]">
      ...
      <div className="flex items-center justify-between mt-5">
        <div className="flex flex-col gap-[1px] w-[50%]">
          ...
          <button className="flex items-center gap-1 border mt-2 text-sm text-slate-600 font-medium px-2 py-1 rounded-full w-fit">
            <span>More Details</span> <FaAngleRight className="text-xs mt-[1.5px]" />
          </button>
        </div>
        ...
        <div className="relative">
          ...
          <button className="absolute bottom-[-15px] left-[11%] text-green-600 font-extrabold bg-white shadow-md px-10 py-2 rounded-lg hover:bg-slate-200 transition-all">ADD</button>
        </div>
      </div>
    </div>
  );
